...
class DNSRecordsRepository:
    ...
    def __init__(self, db_path: str | None):
        ...
        self.logger.debug(f"Initialized DNSRecordsRepository with DB path: {self.db_path}")
        self._initialize_db()
    ...
    def _initialize_db(self):
        ...
        conn.commit()
        conn.close()
        self.logger.debug("Database initialized and ensured dns_records table exists.")
        
    def list_by(self, zone_id: str, dns_record_id: str, record_name: str) -> List[dict] | None:
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute('''
            SELECT zone_id, dns_record_id, record_name, ip, updated_at
            FROM dns_records
    ...
    def save(self, zone_id: str, dns_record_id: str, record_name: str, ip: str) -> None:
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute('''
            INSERT INTO dns_records (zone_id, dns_record_id, record_name, ip)
            VALUES (?, ?, ?, ?)
        ''', (zone_id, dns_record_id, record_name, ip))
        conn.commit()
        conn.close()
        self.logger.debug(f"Saved DNS record: zone_id={zone_id}, dns_record_id={dns_record_id}, record_name={record_name}, ip={ip}")